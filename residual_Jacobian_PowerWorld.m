...
for x=1:length(theta)
    ...
    %% Calculate residual based on PQ_Flag.
    if(PQ_Flag==0)
        r(:,x)=data_event.Data(:,ndxP_event)-data_Jacobian.Data(:,ndxP);
    elseif(PQ_Flag==1)
        r(:,x)=data_event.Data(:,ndxQ_event)-data_Jacobian.Data(:,ndxQ);
    elseif(PQ_Flag==2)
        r(:,x)=[data_event.Data(:,ndxP_event);data_event.Data(:,ndxQ_event)]-[data_Jacobian.Data(:,ndxP);data_Jacobian.Data(:,ndxQ);];
    end
